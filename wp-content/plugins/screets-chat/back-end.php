 ...
 * @return array $columns
 */
function fn_schat_check_for_updates() {
	$old_version = get_option( 'cx_version' );
	$last_version = ( !empty( $old_version ) ) ? $old_version : get_option( 'schat_version' );

	// Upgrade the plugin
	if( $last_version != SLC_VERSION ) {
		require_once SLC_PATH . '/core/fn.upgrade.php';

		fn_schat_upgrade( $last_version );
